...
def bfs(graph: Graph, start: int) -> None:
    ...
    """
    colors = [Colors.White for _ in range(graph.size)]
    colors[start] = Colors.Grey
    ...
    while queue:
        ...
        for i in range(graph.size):
            if is_edge(graph, node, i) and colors[i] == Colors.White:
                colors[i] = Colors.Grey
                graph.parent[i] = node
                graph.distance[i] = graph.distance[node] + 1
                queue.append(i)
...
def dfs(graph: Graph, start: int) -> None:
    ...
    graph.parent[start] = None
    visit(graph, start, visited, 1)

def visit(graph: Graph, node: int, visited: List[bool], time: int) -> int:
    graph.discovery_time[node] = time
    time += 1
    visited[node] = True
    for i in range(graph.size):
        if is_edge(graph, node, i) and not visited[i]:

            graph.parent[i] = node
            time = visit(graph, i, visited, time)
